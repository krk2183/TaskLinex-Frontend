...
export default function SignupPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        if (!/^[a-zA-Z0-9_]{3,30}$/.test(Data.username.trim())) {
        ...
        try {
            ...
            // Step 1: Sign up with Supabase — pass all metadata for the DB trigger
            ...
            if (authError) {
                ...
                if (authError.message.toLowerCase().includes("database")) {
            ...
            // Step 2: Wait briefly for DB trigger
            await new Promise((resolve) => setTimeout(resolve, 2000));

            // Step 3: Check if trigger created the user
            ...
            try {
                ...
                if (userResponse.ok) {
                    console.log("✅ User data verified via trigger");
                    userExists = true;
                }
            } catch (_) {
                console.log("⚠️ Could not verify user, will use fallback");
            }
            ...
            if (!userExists) {
                ...
                if (!ensureResponse.ok) {
                    ...
                    console.error("Fallback failed:", errorData);
                    // Don't block — the user is auth-created, just profile setup failed
                    // They can still log in and profile can be fixed later
                    console.warn("User profile setup failed but auth succeeded, proceeding...");
                } else {
                    ...
                    <form className="space-y-5" onSubmit={handleSignup}>
                        ...
                        {loading && (
                            <div className="p-3 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center gap-2 text-violet-400 text-sm">
                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-violet-400 flex-shrink-0" />
